...
if __name__ == '__main__':
    ...
    # -----------------------------------------------------------
    # (NEW) 1. polls.json, poll_options.json 파일 로드
    # -----------------------------------------------------------
    # (경로 수정) 파일이 있는 실제 경로로 수정해주세요.
    # 예: "../json/polls.json" 또는 "data/polls.json"
    POLLS_FILE_PATH = "polls.json"
    OPTIONS_FILE_PATH = "poll_options.json"
    ...
    if not polls_data or not options_data:
        print("!!! 오류: polls.json 또는 poll_options.json 파일을 로드할 수 없습니다. 경로를 확인하세요.")
    else:
        # -----------------------------------------------------------
        ...
        # -----------------------------------------------------------
        # (REFACTORED) 3. 테스트 시나리오 '배치(Batch)' 실행
        # -----------------------------------------------------------
        # (이 리스트가 user_profile_answers 테이블의 '자유 텍스트' 데이터라고 가정합니다.)
        test_queries = [
            {
                "id": "ANS_001",
                "text": "계속된 야근이랑 지긋지긋한 출퇴근길 때문에 너무 지쳤어. 나를 위한 소비를 해야겠는데, 뭐 해야 할지 모르겠네. 돈은 별로 없어. 그냥 맛있는거나 먹을까..."
            },
            {
                "id": "ANS_002",
                "text": "요새 피부가 너무 건조하고 뒤집어져서 스트레스 받아요. 10만원 안쪽으로 성분 좋은 스킨케어 제품 사고 싶어요."
            },
            {
                "id": "ANS_003",
                "text": "친구랑 싸워서 기분 전환이 필요해. 옷이나 살까봐."
            }
        ]
        
        # (REFACTORED) 3-2. for 루프를 통해 모든 쿼리를 순차적으로 처리
        print(f"\n--- 총 {len(test_queries)}개의 자유 텍스트 응답 분석 시작 ---")
        
        results = [] # 모든 결과를 저장할 리스트

        for i, query in enumerate(test_queries):
            
            test_query_text = query["text"]
            test_query_id = query["id"]
            
            print(f"\n--- ({i+1}/{len(test_queries)}) Claude AI에 JSON 구조화 요청 --- (ID: {test_query_id})")
